...
public class DotProductDemo : MonoBehaviour
{
    ...
    private bool CheckInSight(Transform targetTransform)
    {
        ...
        dotProductValue = Vector3.Dot(transform.forward, directionToTarget);

        angleBetween = Mathf.Acos(Mathf.Clamp(dotProductValue, -1f, 1f)) * Mathf.Rad2Deg;

        return angleBetween <= (fieldOfView * 0.5f);
    }
