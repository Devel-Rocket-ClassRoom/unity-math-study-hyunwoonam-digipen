...
public class TRSMatrixDemo : MonoBehaviour
{
    ...
    private void OnDrawGizmos()
    {
        ...
        if (showSteps)
        {
            ...
            //
            //Matrix4x4 rts = r * t * s;
            //Color WorngColor = new Color(0.5f, 0.5f, 0.5f);
            //DrawShape(originPos, rts, WorngColor, "잘못된 순서 (RTS)");
            //
            //Vector3 wrongOrigin = originPos + rts.MultiplyPoint3x4(Vector3.zero);
            //DrawDashedLine(originPos, wrongOrigin, WorngColor);

            Matrix4x4 sMatrix = Matrix4x4.Scale(scale);
            DrawShape(originPos, sMatrix, Color.yellow, "① Scale");

            Matrix4x4 rMatrix = Matrix4x4.Rotate(rotQuat);
            Matrix4x4 rsMatrix = rMatrix * sMatrix;
            DrawShape(originPos, rsMatrix, Color.cyan, "\n② Rotate");
