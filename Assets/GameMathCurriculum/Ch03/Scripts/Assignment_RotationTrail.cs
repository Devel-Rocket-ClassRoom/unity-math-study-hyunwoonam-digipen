...
public class Assignment_RotationTrail : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        Matrix4x4 rotMatrix = Matrix4x4.TRS(transform.position, Quaternion.Euler(0f, 0f, rotationAngle), Vector3.one);

        Vector3 localTipPos = new Vector3(armLength, 0f, 0f);
        lastTipPos = rotMatrix.MultiplyPoint(localTipPos);

        if (trailPositions.Count == 0 || Vector3.Distance(trailPositions[trailPositions.Count - 1], lastTipPos) > 0.1f)
        {
