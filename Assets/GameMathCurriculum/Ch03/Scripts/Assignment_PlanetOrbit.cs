...
public class Assignment_PlanetOrbit : MonoBehaviour
{
    ...
    private float currentPlanetAngle = 0f;
    private float currentSatelliteAngle = 0f;

    private void Start()
    {
        satellite.SetParent(transform);
    }
    ...
    private void Update()
    {
        ...
        Matrix4x4 planetMatrix = Matrix4x4.TRS(orbitCenter, Quaternion.Euler(0f, currentPlanetAngle, 0f), Vector3.one);
        Vector3 pLocalTarget = new Vector3(planetOrbitRadius, 0f, 0f);
        planetWorldPos = planetMatrix.MultiplyPoint(pLocalTarget);
        
        transform.position = planetWorldPos;
        
        if (satellite != null)
        {
            Matrix4x4 satMatrix = Matrix4x4.TRS(planetWorldPos, Quaternion.Euler(0f, currentSatelliteAngle, 0f), Vector3.one);
            Vector3 sLocalTarget = new Vector3(satelliteOrbitRadius, 0f, 0f);
        
            satelliteWorldPos = satMatrix.MultiplyPoint(sLocalTarget);
        ...
        //float playnetAngle = Time.time * planetOrbitSpeed;
        //
        //Matrix4x4 planetOrbitMatrix = Matrix4x4.TRS(
        //    orbitCenter,
        //    Quaternion.Euler(0f, playnetAngle, 0f),
        //    Vector3.one
        //    );
        //
        ...
        //    );
        //
        //    transform.position = satelliteOrbitMatrix.MultiplyPoint(new Vector3(satelliteOrbitRadius, 0f, 0f));
        //}
        //
