...
public class Assignment_GachaSimulator : MonoBehaviour
{
    ...
    public bool TryPull(int consecutiveFailures, float baseProbability)
    {
        float effectiveProbability = baseProbability;

        if (consecutiveFailures >= hardPity)
        {
            currentEffectiveRate = 1.0f;
            return true;
        }

        if (consecutiveFailures >= softPityStart)
        {
            float t = (float)(consecutiveFailures - softPityStart) / (hardPity - softPityStart);
            effectiveProbability = Mathf.Lerp(baseProbability, 1.0f, t);
        }


        return Random.value < Mathf.Clamp01(effectiveProbability);
    }
